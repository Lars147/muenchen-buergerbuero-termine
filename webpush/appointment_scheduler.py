...
from models import Appointment, SessionLocal, WebPushSubscription
        ...
        with SessionLocal() as db:
            ...
            for appointment in new_appointments:
                ...
                for sub in subscriptions:
                    ...
                    # Match location/office if specified in preferences
                    if offices and str(appointment.office_id) not in offices:
                        matches = False

                    # Match service if specified in preferences
                ...
                for sub in matching_subscriptions:
                    try:
                        # Create booking URL similar to what's in ICS files
                        booking_url = f"https://stadt.muenchen.de/buergerservice/terminvereinbarung.html#/services/{appointment.service_id}/"

                        # Create notification payload
                        payload = json.dumps(
                            {
                                "title": "New Appointment Available!",
                                "message": f"{appointment.get_service_name()} at {appointment.get_office_name()} on {appointment.date.strftime('%Y-%m-%d %H:%M')}",
                                "url": booking_url,
                            }
                        )
...
def sync_appointments_to_db():
    ...
    with SessionLocal() as db:
        ...
        for service_name, offices in data.items():
            # Get office enum from constants
            service_enum: Services = getattr(Services, service_name)

            for office_name, dates in offices.items():
                # Get office enum from constants
                office_enum: Office = getattr(Office, office_name)
                    ...
                    for timestamp in appointment_data["appointmentTimestamps"]:
                        ...
                        existing_appointment = (
                            ...
                            .filter(
                                ...
                                Appointment.office_id == office_enum.office_id,
                                Appointment.service_id == service_enum.value,
