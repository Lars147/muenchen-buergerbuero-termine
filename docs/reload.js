...
window.addEventListener("beforeinstallprompt", (e) => {
	...
	installButton.style.backgroundColor = "#4a89dc";
	...
	// Insert install button next to subscribe button
	const subscribeButton = document.getElementById("subscribeButton");
	if (subscribeButton && !document.getElementById("install-button")) {
...
window.addEventListener("appinstalled", (evt) => {
	console.log("Application was installed to the home screen");
...
function showReloadNotification(newTimestamp) {
	...
	if (!notificationDiv) {
		...
		notificationDiv.style.margin = "0"; // Remove default margin
		notificationDiv.style.backgroundColor = "#e6ffe6";
		notificationDiv.style.border = "1px solid #4caf50";
		notificationDiv.style.borderRadius = "0"; // Make it blend with the top
		notificationDiv.style.position = "sticky"; // Use sticky positioning
		notificationDiv.style.top = "0"; // Stick to the top
		notificationDiv.style.zIndex = "1000"; // Ensure it's on top of other elements
...
function displayAppointments(data, metadataTimestamp) {
	...
	for (const [serviceName, officeData] of Object.entries(data)) {
    ...
    `;
		...
		for (const [officeName, dateData] of Object.entries(officeData)) {
...
async function checkForUpdates() {
	...
	try {
		...
		if (
			lastGithubTimestamp &&
			metadataTimestamp &&
			metadataTimestamp !== lastGithubTimestamp
		) {
